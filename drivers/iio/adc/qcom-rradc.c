...
static bool rradc_is_usb_present(struct rradc_chip *chip)
{
	...
	if (!chip->usb_trig) {
		dev_info(chip->dev, "USB property not present\n");
		return usb_present;
	}
			...
			struct rradc_chan_prop *prop, u16 *data)
{
	...
	case RR_ADC_SKIN_TOO_HOT_TEMP:
		dev_info(chip->dev, "Read only the data registers\n");
		break;
	default:
		status = rradc_chans[prop->channel].sts;
		rc = rradc_read(chip, status, buf, 1);
		...
		rc = rradc_check_status_ready_with_retry(chip, prop,
						buf, status);
		if (rc < 0) {
			dev_info(chip->dev, "Status read failed:%d\n", rc);
			rc = -ENODATA;
			goto fail;
		}
		break;
			 ...
			 long mask)
{
	...
	dev_info(&indio_dev->dev, "%s(), chan = %d", __func__, chan->channel);
...
static int rradc_probe(struct platform_device *pdev)
{
	...
	if (!chip->usb_trig)
		dev_info(chip->dev, "Error obtaining usb power supply\n");
