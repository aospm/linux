...
static bool rradc_is_usb_present(struct rradc_chip *chip)
{
	...
	if (!chip->usb_trig) {
		dev_warn(chip->dev, "USB property not present\n");
		return usb_present;
	}
		...
		struct rradc_chan_prop *prop, u8 *buf, u16 status)
{
	...
	while (((buf[0] & mask) != mask) &&
		...
		dev_dbg(chip->dev, "%s is not ready; nothing to read:0x%x\n",
		...
		if (((prop->channel == RR_ADC_CHG_TEMP) ||
			...
			dev_dbg(chip->dev, "USB not present for %d\n", prop->channel);
			rc = -ENODATA;
			break;
		}
			...
			struct rradc_chan_prop *prop, u16 *data)
{
	...
	case RR_ADC_SKIN_TOO_HOT_TEMP:
		dev_dbg(chip->dev, "Read only the data registers\n");
		break;
	default:
		status = rradc_chans[prop->channel].sts;
		rc = rradc_read(chip, status, buf, 1);
		...
		rc = rradc_check_status_ready_with_retry(chip, prop,
						buf, status);
		if (rc < 0) {
			dev_dbg(chip->dev, "Status read failed:%d\n", rc);
			rc = -ENODATA;
			goto fail;
		}
		break;
			 ...
			 long mask)
{
	...
	dev_dbg(&indio_dev->dev, "%s(), chan = %d", __func__, chan->channel);
...
static int rradc_probe(struct platform_device *pdev)
{
	...
	if (!chip->usb_trig)
		dev_dbg(chip->dev, "Error obtaining usb power supply\n");

	rc = devm_iio_device_register(dev, indio_dev);
	pr_info("RRADC: Called devm_iio_device_register, rc = %d", rc);
	return rc;
}
