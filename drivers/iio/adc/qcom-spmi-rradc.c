...
static int rradc_masked_write(struct rradc_chip *chip, u16 offset, u8 mask,
						u8 val)
{
	int rc;

	rc = regmap_update_bits(chip->regmap, chip->base + offset,
								mask, val);
	if (rc) {
...
static int rradc_read(struct rradc_chip *chip, u16 offset, u8 *data, int len)
{
	...
	while (retry_cnt < FG_RR_ADC_COHERENT_CHECK_RETRY) {
		rc = regmap_bulk_read(chip->regmap, chip->base + offset,
							data, len);
		if (rc < 0) {
...
static const struct rradc_channels rradc_chans[] = {
	{
		...
		.lsb = FG_ADC_RR_BATT_ID_5_LSB,
		.msb = FG_ADC_RR_BATT_ID_5_MSB,
		.sts = FG_ADC_RR_BATT_ID_STS,
	},
	{
		...
		.lsb = FG_ADC_RR_BATT_THERM_LSB,
		.msb = FG_ADC_RR_BATT_THERM_MSB,
		.sts = FG_ADC_RR_BATT_THERM_STS,
	},
	{
		...
		.info_mask = BIT(IIO_CHAN_INFO_PROCESSED),
		.scale = rradc_post_process_therm,
		.lsb = FG_ADC_RR_SKIN_TEMP_LSB,
		.msb = FG_ADC_RR_SKIN_TEMP_MSB,
		.sts = FG_ADC_RR_AUX_THERM_STS,
	},
	{
		...
		.lsb = FG_ADC_RR_USB_IN_I_LSB,
		.msb = FG_ADC_RR_USB_IN_I_MSB,
		.sts = FG_ADC_RR_USB_IN_I_STS,
	},
	{
		...
		.lsb = FG_ADC_RR_USB_IN_V_LSB,
		.msb = FG_ADC_RR_USB_IN_V_MSB,
		.sts = FG_ADC_RR_USB_IN_V_STS,
	},
	{
		...
		.lsb = FG_ADC_RR_DC_IN_I_LSB,
		.msb = FG_ADC_RR_DC_IN_I_MSB,
		.sts = FG_ADC_RR_DC_IN_I_STS,
	},
	{
		...
		.info_mask = BIT(IIO_CHAN_INFO_PROCESSED),
		.scale = rradc_post_process_volt,
		.lsb = FG_ADC_RR_DC_IN_V_LSB,
		.msb = FG_ADC_RR_DC_IN_V_MSB,
		.sts = FG_ADC_RR_DC_IN_V_STS,
	},
	{
		...
		.lsb = FG_ADC_RR_PMI_DIE_TEMP_LSB,
		.msb = FG_ADC_RR_PMI_DIE_TEMP_MSB,
		.sts = FG_ADC_RR_PMI_DIE_TEMP_STS,
	},
	{
		...
		.lsb = FG_ADC_RR_CHARGER_TEMP_LSB,
		.msb = FG_ADC_RR_CHARGER_TEMP_MSB,
		.sts = FG_ADC_RR_CHARGER_TEMP_STS,
	},
	{
		.datasheet_name = "gpio",
		.type = IIO_VOLTAGE,
		...
		.lsb = FG_ADC_RR_GPIO_LSB,
		.msb = FG_ADC_RR_GPIO_MSB,
		.sts = FG_ADC_RR_GPIO_STS,
	},
	{
		...
		.lsb = FG_ADC_RR_CHARGER_HOT,
		.msb = FG_ADC_RR_CHARGER_HOT,
		.sts = FG_ADC_RR_CHARGER_TEMP_STS,
	},
	{
		...
		.info_mask = BIT(IIO_CHAN_INFO_PROCESSED),
		.scale = rradc_post_process_chg_temp_hot,
		.lsb = FG_ADC_RR_CHARGER_TOO_HOT,
		.msb = FG_ADC_RR_CHARGER_TOO_HOT,
		.sts = FG_ADC_RR_CHARGER_TEMP_STS,
	},
	{
		...
		.lsb = FG_ADC_RR_SKIN_HOT,
		.msb = FG_ADC_RR_SKIN_HOT,
		.sts = FG_ADC_RR_AUX_THERM_STS,
	},
	{
		...
		.info_mask = BIT(IIO_CHAN_INFO_PROCESSED),
		.scale = rradc_post_process_skin_temp_hot,
		.lsb = FG_ADC_RR_SKIN_TOO_HOT,
		.msb = FG_ADC_RR_SKIN_TOO_HOT,
		.sts = FG_ADC_RR_AUX_THERM_STS,
	},
};
		...
		struct rradc_chan_prop *prop, u8 *buf, u16 status)
{
	...
	while (((buf[0] & mask) != mask) &&
		...
		dev_info(chip->dev, "%s is not ready; nothing to read:0x%x\n",
