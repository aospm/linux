...
/**
 * spmi_fg_read() - Read multiple registers with regmap_bulk_read
 * 
 ...
 */
static int spmi_fg_read(struct regmap *map, u8 *val, u16 addr, int len)
{
	if ((addr & 0xff00) == 0) {
...
/**
 * @brief spmi_fg_write() - Write multiple registers with regmap_bulk_write
 * 
 ...
 * @return int 0 on success, negative errno on error
 */
static int spmi_fg_write(struct regmap *map, u8 *val, u16 addr, int len)
{
	int rc;
...
/**
 * @brief spmi_fg_masked_write() - like spmi_fg_write but applies
 * a mask first.
 * 
 * @param map The regmap to write
 * @param val Pointer to write values into
 ...
 * @return int 0 on success, negative errno on error
 */
static int spmi_fg_masked_write(struct regmap *map, u16 addr,
		u8 mask, u8 val)
{
	int error;
	u8 reg;
	error = spmi_fg_read(map, &reg, addr, 1);
	if (error)
		return error;
	...
	error = spmi_fg_write(map, &reg, addr, 1);
	return error;
}
...
static int spmi_fg_fg_get_capacity(struct spmi_fg_fg_chip *chip, int *val)
{
	u8 cap[2];
	int error = spmi_fg_read(chip->regmap, cap, REG_BASE + BATT_MONOTONIC_SOC, 2);
	if (error)
		return error;
	if (cap[0] != cap[1]) {
...
static int spmi_fg_fg_get_temperature(struct spmi_fg_fg_chip *chip, int *val)
{
	int rc, temp;
	u8 readval[2];

	rc = spmi_fg_read(chip->regmap, readval, REG_BATT + PARAM_ADDR_BATT_TEMP, 2);
	if (rc) {
...
static int spmi_fg_fg_get_current(struct spmi_fg_fg_chip *chip, int *val)
{
	int rc, temp;
	u8 readval[2];

	rc = spmi_fg_read(chip->regmap, readval, REG_BATT + PARAM_ADDR_BATT_CURRENT, 2);
	if (rc) {
...
static int spmi_fg_fg_get_voltage(struct spmi_fg_fg_chip *chip, int *val)
{
	int rc, temp;
	u8 readval[2];

	rc = spmi_fg_read(chip->regmap, readval, REG_BATT + PARAM_ADDR_BATT_VOLTAGE, 2);
	if (rc) {
...
static int spmi_fg_iacs_clear_sequence(struct spmi_fg_fg_chip *chip)
{
	int rc = 0;
	u8 temp;

	/* clear the error */
	rc = spmi_fg_masked_write(chip->regmap, REG_MEM + MEM_INTF_IMA_CFG,
				BIT(2), BIT(2));
	if (rc) {
	...
	temp = 0x4;
	rc = spmi_fg_write(chip->regmap, &temp, REG_MEM + MEM_INTF_ADDR_LSB + 1, 1);
	if (rc) {
	...
	temp = 0x0;
	rc = spmi_fg_write(chip->regmap, &temp, REG_MEM + MEM_INTF_WR_DATA0 + 3, 1);
	if (rc) {
		pr_err("Error writing to WR_DATA3, rc=%d\n", rc);
		return rc;
	}

	rc = spmi_fg_read(chip->regmap, &temp, REG_MEM + MEM_INTF_RD_DATA0 + 3, 1);
	if (rc) {
...
static int spmi_fg_clear_ima(struct spmi_fg_fg_chip *chip,
		bool check_hw_sts)
{
	int rc = 0, ret = 0;
	...
	rc = spmi_fg_read(chip->regmap, &err_sts,
	...
	rc = spmi_fg_read(chip->regmap, &exp_sts,
	...
	if (check_hw_sts) {
		rc = spmi_fg_read(chip->regmap, &hw_sts,
	...
	if (run_err_clr_seq) {
		ret = spmi_fg_iacs_clear_sequence(chip);
		if (!ret)
			return -EAGAIN;
		else
...
int spmi_fg_get_prop_usb_online(struct spmi_fg_fg_chip *chip, int *val){
	unsigned int stat;
	int rc;
...
int spmi_fg_get_prop_batt_status(struct spmi_fg_fg_chip *chip, int *val){
	int usb_online_val;
	unsigned int stat;
	int rc;
	bool usb_online;

	rc = spmi_fg_get_prop_usb_online(chip, &usb_online_val);
	if (rc < 0) {
...
int spmi_fg_get_prop_health_status(struct spmi_fg_fg_chip *chip, int *val){
	unsigned int stat;
	int rc;
...
static int spmi_fg_get_temp_threshold(struct spmi_fg_fg_chip *chip,
				enum power_supply_property psp, int *val)
{
	...
	rc = spmi_fg_read(chip->regmap, &temp, reg, 1);
	if (rc < 0) {
...
static void fg_get_model_name(struct spmi_fg_fg_chip *chip, union power_supply_propval *val)
{
	switch (chip->subtype)
	{
...
{
	struct spmi_fg_fg_chip *chip = power_supply_get_drvdata(psy);
	int error = 0;
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		error = spmi_fg_fg_get_capacity(chip, &val->intval);
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		error = spmi_fg_fg_get_current(chip, &val->intval);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		error = spmi_fg_fg_get_voltage(chip, &val->intval);
		break;
	...
	case POWER_SUPPLY_PROP_STATUS:
		error = spmi_fg_get_prop_batt_status(chip, &val->intval);
		break;
	case POWER_SUPPLY_PROP_HEALTH:
		error = spmi_fg_get_prop_health_status(chip, &val->intval);
		break;
	...
	case POWER_SUPPLY_PROP_TEMP:
		error = spmi_fg_fg_get_temperature(chip, &val->intval);
		break;
	case POWER_SUPPLY_PROP_TEMP_MIN:
	case POWER_SUPPLY_PROP_TEMP_MAX:
	case POWER_SUPPLY_PROP_TEMP_ALERT_MIN:
	case POWER_SUPPLY_PROP_TEMP_ALERT_MAX:
		error = spmi_fg_get_temp_threshold(chip, psp, &val->intval);
		break;
...
irqreturn_t spmi_fg_handle_usb_plugin(int irq, void *data){
	struct spmi_fg_fg_chip *chip = data;
	int rc;
	unsigned int stat;
	bool vbus_rising;
	union power_supply_propval val;
...
static int spmi_fg_fg_probe(struct platform_device *pdev)
{
	struct power_supply_config supply_config = {};
	struct spmi_fg_fg_chip *chip;
	...
	rc = spmi_fg_read(chip->regmap, chip->revision, REG_MEM + DIG_MINOR, 4);
	if (rc) {
	 ...
	 */
	rc = spmi_fg_masked_write(chip->regmap,
	...
	rc = spmi_fg_clear_ima(chip, true);
	if (rc && rc != -EAGAIN) {
	...
	rc = spmi_fg_read(chip->regmap, &dma_status, REG_MEM + 0x70, 1);
	if (rc < 0) {
	...
	rc = spmi_fg_masked_write(chip->regmap, REG_MEM + 0x71, BIT(0),
